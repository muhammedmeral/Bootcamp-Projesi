...
public class FotoCekme : MonoBehaviour
{
    ...
    public int fotoCekildiMi;   //Foto çekilirse 1 çekilmezse 0
    ...
    private void Start()
    {
        ...
        audioSource = GetComponent<AudioSource>(); //audiosource componentini cache ettik.s
        fotoCekildiMi = 0;
        PlayerPrefs.DeleteKey("fotoCekildiMi");
    }
        ...
        if (Input.GetMouseButtonDown(0) && fotoCekmeLimiti > 0 && PauseMenu.oyunDurduMu == false)
        {
            if (Time.time - sonTiklama > tiklamaBeklemeSuresi)
            {
                ...
                if (KadrajdaMi() == true)
                {
                    Kaydet();
                    fotoCekildiMi = 1;
                    PlayerPrefs.SetInt("fotoCekildiMi", fotoCekildiMi);  //Player prefsle veriyi tuttuk
                    fotoCekmeLimiti = 0;
                }
    ...
    public int fotoCekmeLimt()
    {
        return fotoCekmeLimiti;
    }
}